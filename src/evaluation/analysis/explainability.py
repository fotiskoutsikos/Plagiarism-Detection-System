...
from utils.constants import PLOT_STYLE_PARAMS, CATEGORY_COLORS, OUTPUT_DIRS
...
TOP_K = 30
RES_SUBDIR = OUTPUT_DIRS.get("explainability", "results/explainability")
PLT_SUBDIR = OUTPUT_DIRS.get("explainability_plots", "plots/explainability")
...
def plot_overall_shifts(df: pd.DataFrame, model_name: str, plt_dir: str) -> None:
    ...
    for cat in categories:
        ...
        ax1.hist(vals, bins=40, alpha=0.5, color=_cat_color(cat), label=_short(cat), density=True)
    ...
    plt.suptitle(f"{model_name} — Shift Magnitudes & Active Dimensions", fontsize=14, fontweight="bold", y=1.02)
    plt.tight_layout()
    _save(fig, os.path.join(plt_dir, f"{model_name.lower()}_overall_shifts.pdf"))


# PLOTS 2 & 3: Top-K Affected Dimensions & Overlap
def plot_topk_and_overlap(df: pd.DataFrame, model_name: str, res_dir: str, plt_dir: str) -> None:
    print(f"Generating Plot 2 & 3: Top-{TOP_K} Dimensions & Overlap…")
    ...
    for cat in categories:
        ...
        for rank, dim in enumerate(top_idx, 1):
            topk_records.append({
                "category": cat, "rank": rank, "dimension": int(dim),
                "mean_delta": float(cat_mean[dim]), "std_delta": float(cat_std[dim])
            })
    ...
    for idx, cat in enumerate(categories):
        ...
        ax.bar(np.arange(TOP_K), cd["mean_delta"].values, yerr=cd["std_delta"].values, capsize=2,
    ...
    plt.suptitle(f"{model_name} — Dimensions Most Affected per Category", fontsize=14, fontweight="bold", y=1.01)
    plt.tight_layout()
    ...
    ax_ov.set_title(f"{model_name} — Top-{TOP_K} Dimension Overlap Between Categories", fontweight="bold", pad=15)
...
def plot_signed_shift(df: pd.DataFrame, model_name: str, plt_dir: str) -> None:
    ...
    im = ax.imshow(shift_vis, aspect="auto", cmap="RdBu_r", vmin=-vmax, vmax=vmax, interpolation="nearest")
    ...
    ax.set_title(f"{model_name} — Signed Mean Shift per Dimension {subtitle}", fontweight="bold", fontsize=13)
    plt.colorbar(im, ax=ax, shrink=0.8, pad=0.02, label="Mean Signed Shift (mod − ori)")
    plt.tight_layout()
    _save(fig, os.path.join(plt_dir, f"{model_name.lower()}_signed_shift_heatmap.pdf"))


# PLOT 5: SMP vs AI Discrimination
def plot_smp_vs_ai(df: pd.DataFrame, model_name: str, res_dir: str, plt_dir: str) -> None:
    ...
    pooled_std = np.sqrt((np.std(smp_delta, axis=0)**2 + np.std(ai_delta, axis=0)**2) / 2 + 1e-10)
    ...
    _csv(comparison_df, os.path.join(res_dir, f"{model_name.lower()}_smp_vs_ai.csv"))
    ...
    ax2.text(0.95, 0.95, f"Large Effect (|d| > 0.8):\n{n_sig} dimensions", 
    ...
    plt.suptitle(f"{model_name} — SMP vs AI Base: Dimension-Level Comparison", fontsize=14, fontweight="bold", y=1.02)
    plt.tight_layout()
    _save(fig, os.path.join(plt_dir, f"{model_name.lower()}_smp_vs_ai.pdf"))


# PLOT 6: Stable Core Preservation
def plot_stable_core(df: pd.DataFrame, model_name: str, plt_dir: str) -> None:
    logger.info("Generating Plot 6: Stable Core Preservation…")

    delta_matrix = np.stack(df["delta_vector"].values)
    global_mean = np.mean(delta_matrix, axis=0)
    
    # Define Core and Shell based on global stability
    ...
    ax.set_title(f"{model_name} — Stable Core Preservation\n"
                 "(Plagiarism retains content while disrupting style)", 
                 fontweight="bold", fontsize=13)
    ax.grid(axis='y', linestyle='--', alpha=0.7)

    # Custom Legend (since colors now represent categories, not stable/volatile)
    legend_elements = [
    ...
    _save(fig, os.path.join(plt_dir, f"{model_name.lower()}_stable_core.pdf"))
...
def process_model(parquet_path: str, pairs_path: str, model_name: str) -> None:
    ...
    # 1. Overall Shifts & Active Dims
    plot_overall_shifts(df, model_name, PLT_SUBDIR)
    
    # 2 & 3. Top-K Dimensions & Overlap Heatmap
    plot_topk_and_overlap(df, model_name, RES_SUBDIR, PLT_SUBDIR)
    
    # 4. Signed Mean Shift Heatmap (first 200 + last 200)
    plot_signed_shift(df, model_name, PLT_SUBDIR)
    
    # 5. SMP vs AI Comparison (Scatter & Cohen's d)
    plot_smp_vs_ai(df, model_name, RES_SUBDIR, PLT_SUBDIR)
    
    # 6. Stable Core Preservation (Content vs Style)
...
def main() -> None:
    ...
    for model_name, parquet in MODELS:
        if os.path.exists(parquet) and os.path.exists(PAIRS_CSV):
            process_model(parquet, PAIRS_CSV, model_name)
        else:
