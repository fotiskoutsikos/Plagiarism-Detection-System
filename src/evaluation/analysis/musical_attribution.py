...
Outputs:
  ...
  - Per-model MGE-LDM stem attribution CSV + bar plot (supplementary)
...
def build_query_groups(df: pd.DataFrame, metric_col: str):
    ...
    Grouping key: (pair_id, time, filename_ori, clean_mod_type)

    Each group contains exactly 1 positive + 3 negatives (one per tier).
    Deterministic tie-breaking: smallest filename_mod within each role.
    """
    required_cols = [
...
def rank_queries(df_valid: pd.DataFrame, metric_col: str) -> pd.DataFrame:
    ...
    for query_key, grp in df_valid.groupby("query_key"):
        ...
        pos_idx       = pos_mask.idxmax()
        positive_rank = int(pos_idx) + 1

        rank1_row   = grp_sorted.iloc[0]
        winner_tier = "correct" if rank1_row["y_true"] == 1 else str(rank1_row["negative_tier"])
        ...
        results.append({
            ...
            "rank1_tier": (
                str(rank1_row["negative_tier"])
                if rank1_row["y_true"] == 0
                else POSITIVE_TIER_LABEL
            ),
        })
...
def compute_attribution_metrics(
    df_ranked: pd.DataFrame, label: str = "Overall"
) -> dict:
...
def compute_grouped_metrics(
    df_ranked: pd.DataFrame, group_col: str
) -> pd.DataFrame:
    ...
    for group_val in sorted(df_ranked[group_col].unique()):
        ...
        rows.append(compute_attribution_metrics(subset, label=str(group_val)))
...
def compute_misattribution_breakdown(df_ranked: pd.DataFrame) -> pd.DataFrame:
    ...
    won rank 1, broken down by broad category.
    """
...
# STEM HELPERS
def _build_stem_ranked(df_ranked: pd.DataFrame) -> pd.DataFrame:
    """
    From the ranked results, extract MGE-LDM rows and attach stem labels.

    Uses extract_features_with_stem() on clean_mod_type so that the stem
    is derived from the canonical label, not a raw filename heuristic.

    Returns a copy of df_ranked filtered to MGE-LDM rows, with a new
    'stem' column. Rows where stem is not in MGELDM_STEM_ORDER are dropped.
    """
    df_mgeldm = df_ranked[
        df_ranked["clean_mod_type"].str.contains("mgeldm", case=False, na=False)
    ].copy()

    if df_mgeldm.empty:
        return pd.DataFrame()

    stem_series = df_mgeldm["clean_mod_type"].apply(
        lambda m: extract_features_with_stem(m).iloc[1]   # index 1 = stem
    )
    df_mgeldm["stem"] = stem_series
...
def plot_rank_distribution(
    ...
    for r in range(1, ATTRIBUTION_GROUP_SIZE + 1):
        ...
        ax.bar(
            ...
            label=f"Rank {r}", color=ATTRIBUTION_RANK_COLORS[r],
            edgecolor="white", linewidth=0.5,
        )
        for i, (val, bot) in enumerate(zip(values, bottom)):
            if val >= 5:
                ax.text(
                    x[i], bot + val / 2, f"{val:.0f}%",
                    ha="center", va="center",
...
def plot_misattribution_breakdown(
    ...
    bars        = ax.barh(display_names, overall["count"].values,
                          color=colors, edgecolor="white", linewidth=0.8)
    ...
    ax.set_title(f"Misattribution Error Source — {model_name}", fontsize=13, fontweight="bold")
...
def plot_stem_attribution(
    ...
    """
    Supplementary bar chart: Top-1 Attribution Accuracy per MGE-LDM stem.

    Shows top1_accuracy for bass / drums / other alongside MRR and mean rank
    as text annotations, so the reader can compare stem-level retrievability
    at a glance.

    Saved as: {model}_attribution_stem_top1.pdf
    """
    if df_stem.empty or "top1_accuracy" not in df_stem.columns:
        return

    df_plot = df_stem[df_stem["label"].isin(MGELDM_STEM_ORDER)].copy()
    # Preserve canonical stem order
    df_plot["_order"] = df_plot["label"].map(
        {s: i for i, s in enumerate(MGELDM_STEM_ORDER)}
    )
    ...
    colors  = [MGELDM_STEM_COLORS.get(s, "#999999") for s in df_plot["label"]]
    bars    = ax.bar(
        range(len(df_plot)),
        df_plot["top1_accuracy"].values * 100,
    ...
    for bar, (_, row) in zip(bars, df_plot.iterrows()):
        ...
        mrr  = row.get("mrr", float("nan"))
        mr   = row.get("mean_rank", float("nan"))
    ...
    ax.set_xticks(range(len(df_plot)))
    ax.set_xticklabels(
        [s.capitalize() for s in df_plot["label"].values],
        fontsize=12,
    )
    ax.set_ylabel("Top-1 Attribution Accuracy (%)", fontsize=11, fontweight="bold")
    ax.set_title(
        f"MGE-LDM Stem Attribution — {model_name}\n"
        f"(metric: {metric_name}, aggregated over all DSP variants)",
    ...
    path = output_dir / f"{model_name.lower()}_attribution_stem_top1.pdf"
    fig.savefig(path, dpi=PLOT_DPI, format="pdf", bbox_inches="tight")
    plt.close(fig)
    print(f"  Plot saved → {path.name}")
...
def _print_stem_table(df_stem: pd.DataFrame, model_name: str) -> None:
    """Print MGE-LDM stem attribution summary to console."""
    if df_stem.empty:
        return
    print(f"\n  {'─' * 70}")
    print(f"  MGE-LDM Stem Attribution — {model_name}")
    print(
        f"  {'Stem':<10} | {'Top-1':>7} | {'MRR':>7} | "
        f"{'MeanR':>7} | {'N':>5}"
    )
    print(f"  {'─' * 50}")
    for _, row in df_stem.iterrows():
        if row["label"] not in MGELDM_STEM_ORDER:
            continue
        print(
            f"  {row['label']:<10} | {row['top1_accuracy']:>6.1%} | "
            f"{row['mrr']:>7.4f} | {row['mean_rank']:>7.3f} | "
            f"{row['n_queries']:>5}"
        )
    print(f"  {'─' * 70}")
...
def run_model_attribution(
    ...
    Main section  : overall, broad-category, granular, misattribution.
    Stem section  : MGE-LDM rows only, grouped by stem (supplementary).
    ...
    if not df_misattr.empty:
        print("  Misattribution Breakdown:")
        total_errors = df_misattr["count"].sum()
        for tier, cnt in df_misattr.groupby("winner_tier")["count"].sum().items():
            tier_display = ATTRIBUTION_TIER_DISPLAY.get(tier, tier)
            pct = cnt / total_errors * 100 if total_errors > 0 else 0
            print(f"    {tier_display:<25}: {cnt:>4} ({pct:.1f}%)")
    ...
    # Supplementary: MGE-LDM stem attribution────────────────────────────
    df_stem_ranked = _build_stem_ranked(df_ranked)

    if df_stem_ranked.empty:
        print(f"  No MGE-LDM rows in ranked results — stem attribution skipped.")
    else:
        n_stem = len(df_stem_ranked)
        stem_counts = df_stem_ranked["stem"].value_counts().to_dict()
        print(f"\n  MGE-LDM stem rows: {n_stem} {stem_counts}")

        # Metrics grouped by stem (aggregated over all DSP variants)
        df_stem_metrics = compute_grouped_metrics(df_stem_ranked, "stem")

        _print_stem_table(df_stem_metrics, model_name)

        # CSV
        df_stem_metrics.to_csv(
            output_dir / f"{model_name.lower()}_attribution_stem.csv", index=False
        )
        print(
            f"  Stem CSV saved → "
            f"{model_name.lower()}_attribution_stem.csv"
        )

        # Plot
        plot_stem_attribution(df_stem_metrics, model_name, metric_col, plots_dir)
...
def main() -> None:
    ...
    for model_name, csv_path in MODEL_PATHS.items():
        ...
        if result:
            all_summaries.append({"model": model_name, "metric": metric_col, **result})
    ...
    if all_summaries:
        ...
        print(f"\n{'=' * 80}")
        print(" CROSS-MODEL ATTRIBUTION COMPARISON (4-Way Forced Choice)")
        print(f"{'=' * 80}")
        print(
            f"  {'Model':<10} | {'Metric':<25} | {'Top-1':>7} | "
